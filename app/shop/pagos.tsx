...
import { ProductosDePagaB_Public } from '@/routes/public.routes';
...
export default function PagoScreen() {
    ...
    useEffect(() => {
        const fetchConsulta = async (page: number, search: string) => {
            const { alert, data, message } = await get(`${ProductosDePagaB_Public}?page=${page}&limit=${itemsPerPage}&search=${search}&order=desc`, { "Content-Type": "application/json", });
            setError(null);
            setLoading(false);
            if (alert === "success") {
