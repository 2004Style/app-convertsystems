...
export default function OfertasScreen() {
  ...
  // Filtrar productos en oferta (tienen plan asociado y descuento)
  const productosEnOferta = seed_productos.filter(producto =>
    producto.plan !== null && producto.precio_actual.descuento > 0
  );
  const productosFiltrados = productosEnOferta.filter(producto =>
  ...
  const handleBuy = (producto: Productos) => {
    Alert.alert(
      'Comprar Produto em Oferta',
      `Deseja comprar "${producto.nombre}" por ${new Intl.NumberFormat('es-ES', {
        style: 'currency',
        currency: 'EUR'
      }).format(producto.precio_actual.precio)}? ¡OFERTA ESPECIAL!`,
      [
        { text: 'Cancelar', style: 'cancel' },
        { text: 'Comprar', onPress: () => console.log('Comprando oferta:', producto.id) }
      ]
    );
  };
  ...
  return (
    <>
      ...
      <AppLayout title="Tienda de Ofertas">
        <ScrollView>
            ...
            {productosFiltrados.length > 0 ? (
              ...
              <View className="flex-1 justify-center items-center py-20">
                <Text className="text-gray-500 text-lg mb-2">
                  {searchText ? 'No se encontraron productos' : 'No hay productos en oferta'}
                </Text>
                <Text className="text-gray-400 text-sm text-center mb-4">
                  {searchText ? 'Intenta con otros términos de búsqueda' : 'Vuelve pronto para ver nuevas ofertas especiales'}
                </Text>
              </View>
            )}
          </ScrollView>
